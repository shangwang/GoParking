...
- (void)viewDidLoad
{
    ...
    self.tableView.tableHeaderView = ({
        ...
        imageView.image = [UIImage imageNamed:@"Shang.jpg"];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)sectionIndex
{
    ...
    UILabel *label2 = [[UILabel alloc] initWithFrame:CGRectMake(10, 8, 0, 0)];
    label2.text = @"其他服务";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0 && indexPath.row == 0) {
        ...
        CarServiceViewController *serviceView=[[CarServiceViewController alloc]initWithNibName:@"CarServiceViewController" bundle:nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (0==indexPath.section) {
        NSArray *titles = @[@"主页", @"我的信息", @"我的好友"];
        cell.textLabel.text = titles[indexPath.row];
    } else if (1==indexPath.section ){
        NSArray *titles = @[@"查询车位", @"导航", @"预约停车"];
        cell.textLabel.text = titles[indexPath.row];
    }else{
        NSArray *titles = @[@"洗车服务", @"代驾服务", @"待泊服务"];
        cell.textLabel.text = titles[indexPath.row];
    }
