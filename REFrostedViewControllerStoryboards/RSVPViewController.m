...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.row==0&&1==indexPath.section){
        StartRSVPCell* cell = [[[NSBundle mainBundle] loadNibNamed:@"StartRSVPCell" owner:self options:nil] lastObject];
        return cell.frame.size.height;

    }
    if(indexPath.row==0&&2==indexPath.section){
        FeeDetailCell* cell = [[[NSBundle mainBundle] loadNibNamed:@"FeeDetailCell" owner:self options:nil] lastObject];
        return cell.frame.size.height;
    }
