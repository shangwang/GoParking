...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withStringArray:(NSArray *)stringArray
{
    ...
    if( [firstString isEqualToString:@"NoteTaking"]){
        NSLog(@"Note taking!");
        noteText = [[UITextView alloc] initWithFrame:CGRectMake(0,0,300,110)];
        noteText.selectedRange=NSMakeRange(0,0) ;
        noteText.font=[UIFont fontWithName:@"MarkerFelt-Thin" size:16.f];
        [noteText becomeFirstResponder];
        [noteText setBackgroundColor:[UIColor clearColor]];
        [labelArray addObject:[noteText AUTORELEASE]];
        viewType=@"NoteTaking";
    ...
    for (NSString *string in stringArray) {
        ...
        CGSize screenSize = [self screenSize];
        UIButton *textButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, textSize.width, textSize.height)];
        //we dont wanna the popup view to exceed the width of the screen
        if(textSize.width>screenSize.width*popViewRatial){
            int heightSize=1;
            while(screenSize.width*popViewRatial* heightSize < textSize.width){
                heightSize++;
            }
            heightSize++;
        textButton=[[UIButton alloc] initWithFrame:CGRectMake(0, 0, screenSize.width*popViewRatial,heightSize*textSize.height)];
        }
        textButton.backgroundColor = [UIColor clearColor];
